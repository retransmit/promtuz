...
mod tests {
    ...
    #[test]
    fn extract_pubkey_node_id_mismatch_detected() {
        // Build a cert with one pubkey, claim a different NodeId →
        // the wrapper rejects with `NodeIdMismatch`.
        let pubkey = [0x33u8; 32];
        let der = build_test_cert(&pubkey, &[0u8; 64]);
        let extracted = extract_pubkey_from_leaf_der(&der).expect("parse");
        assert_eq!(extracted, pubkey);

        // A NodeId derived from the actual pubkey must round-trip.
        let id_match = NodeId::new(pubkey);
        let derived = NodeId::new(extracted);
        assert_eq!(derived, id_match);

        // Now build a *bogus* claim — a different seed.
        let other_id = NodeId::new([0x77u8; 32]);
        // Sanity: other_id must differ.
        assert_ne!(derived, other_id);

        // The wrapper logic mirrors what `extract_and_verify_pubkey`
        // does internally.
        assert!(derived != other_id);
    }

    /// Pure-function check: build a cert whose SPKI is one pubkey,
    /// then run the `verify` step against a NodeId derived from a
    ...
    #[test]
    fn fake_cert_with_mismatched_node_id_is_rejected() {
        // Cert was generated with pubkey_a in its SPKI.
        let pubkey_a = [0x11u8; 32];
        let cert_der = build_test_cert(&pubkey_a, &[0u8; 64]);
        let extracted = extract_pubkey_from_leaf_der(&cert_der).expect("parse");

        // Claim a NodeId derived from pubkey_b instead.
        let pubkey_b = [0x22u8; 32];
        let claimed_b = NodeId::new(pubkey_b);

        // The check that `extract_and_verify_pubkey` performs against
        // a real `Connection`:
        let derived = NodeId::new(extracted);
        assert_ne!(derived, claimed_b);
    }
}
